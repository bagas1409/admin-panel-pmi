...
import { History, X, Phone, Droplets, CheckCircle, AlertCircle, Clock, Search, Users } from 'lucide-react'
...
const getDonorStatus = (histories?: { donationDate: string }[]) => {
    if (!histories || histories.length === 0)
        return { label: 'Belum Pernah Donor', color: '#9CA3AF', bg: '#F3F4F6', icon: 'none', days: null }
    const sorted = [...histories].sort((a, b) => new Date(b.donationDate).getTime() - new Date(a.donationDate).getTime())
    const days = getDaysSince(sorted[0].donationDate)
    if (days === null) return { label: 'Data Tidak Valid', color: '#9CA3AF', bg: '#F3F4F6', icon: 'none', days: null }
    if (days <= 30)   return { label: `Masa Pemulihan (hari ke-${days})`, color: '#059669', bg: '#D1FAE5', icon: 'recovery', days }
    if (days < 90)    return { label: `Hampir Siap (${days} hari)`, color: '#D97706', bg: '#FEF3C7', icon: 'soon', days }
    return { label: `Siap Donor Ulang (${days} hari)`, color: '#DC2626', bg: '#FEE2E2', icon: 'ready', days }
}
...
const dummyDonors: User[] = Array.from({ length: 10 }).map((_, i) => {
    ...
    let daysAgo = 0
    if (i % 3 === 0) daysAgo = Math.floor(Math.random() * 20) + 1      // 1-20 hari -> Masa Pemulihan
    else if (i % 3 === 1) daysAgo = Math.floor(Math.random() * 50) + 31 // 31-80 hari -> Hampir Siap
    ...
    const names = [
        "Budi Santoso", "Siti Aminah", "Andi Suryono", "Rina Marlina", "Ahmad Fauzan",
        "Dewi Lestari", "Eko Prasetyo", "Nina Kusuma", "Rudi Hartono", "Maya Sari"
    ]
...
export default function DonorsPage() {
    ...
    const paginatedDonors = filtered.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
    ...
    const newDonors = combinedDonors.filter(d => !d.donationHistories?.length).length
    ...
    return (
        <div className="space-y-6 relative">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--text)]">Monitoring Relawan Pendonor</h1>
                    <p className="text-sm text-[var(--text-muted)] mt-1">Kelola & pantau kesehatan dan kesiapan seluruh relawan donor PMI</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border)] p-5 flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-2xl font-bold text-[var(--text)]">{totalDonors}</div>
                        <div className="text-sm text-[var(--text-muted)]">Total Relawan Terdaftar</div>
                    </div>
                </div>
                <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border)] p-5 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-red-50 flex items-center justify-center">
                    ...
                    <div>
                        <div className="text-2xl font-bold text-[var(--text)]">{readyDonors}</div>
                        <div className="text-sm text-[var(--text-muted)]">Siap Donor Ulang</div>
                    </div>
                </div>
                <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border)] p-5 flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-2xl font-bold text-[var(--text)]">{newDonors}</div>
                        <div className="text-sm text-[var(--text-muted)]">Belum Pernah Donor</div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border)] overflow-hidden shadow-sm">
                            ...
                            {loading ? (
                                <tr><td colSpan={6} className="px-6 py-12 text-center">
                                    <div className="flex flex-col items-center gap-3 text-gray-400">
                                        ...
                                        <span className="text-sm">Memuat data seluruh relawan...</span>
                                    </div>
                                </td></tr>
                            ) : paginatedDonors.length === 0 ? (
                                <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400 text-sm">
                                    {search ? `Tidak ada relawan yang sesuai dengan pencarian "${search}"` : 'Belum ada relawan terdaftar.'}
                                </td></tr>
                            ) : (
                                paginatedDonors.map(d => {
                                    const status = getDonorStatus(d.donationHistories)
                                    const rhesus = d.donorProfile?.rhesus === 'POSITIVE' ? '+' : d.donorProfile?.rhesus === 'NEGATIVE' ? '-' : ''
                                    const waNumber = d.donorProfile?.whatsappNumber?.replace(/^0/, '62').replace(/\D/g, '')
                                    ...
                                    return (
                                        <tr key={d.id} className="hover:bg-gray-50/60 transition-colors group">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-red-50 border-2 border-red-100 flex items-center justify-center flex-shrink-0">
                                                        <span className="text-[var(--primary)] font-bold text-sm">
                                                            {d.donorProfile?.fullName?.charAt(0) || d.email.charAt(0).toUpperCase()}
                                                        </span>
                                                    </div>
                                                    <div>
                                                        <div className="font-semibold text-[var(--text)] text-sm">
                                                            {d.donorProfile?.fullName || <span className="text-gray-400 italic">Profil Belum Lengkap</span>}
                                                        </div>
                                                        <div className="text-xs text-gray-400">{d.email}</div>
                                                        {d.donorProfile?.nik && (
                                                            <div className="text-xs text-gray-400 font-mono">NIK: {d.donorProfile.nik}</div>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                {d.donorProfile?.bloodType ? (
                                                    <span className="inline-flex items-center bg-red-50 text-[var(--primary)] px-3 py-1.5 rounded-xl text-sm font-bold border border-red-100">
                                                        ...
                                                        {d.donorProfile.bloodType}{rhesus}
                                                    </span>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                >
                                                    {status.icon === 'ready'    && <CheckCircle className="w-3.5 h-3.5" />}
                                                    {status.icon === 'soon'     && <Clock className="w-3.5 h-3.5" />}
                                                    {status.icon === 'recovery' && <AlertCircle className="w-3.5 h-3.5" />}
                                                    {status.label}
                                                </div>
                                                {latestDonation && (
                                                    <div className="text-xs text-gray-400 mt-1 ml-1">
                                                        Terakhir: {new Date(latestDonation.donationDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                                                    </div>
                                                )}
                                            </td>
                ...
                <div className="px-6 py-4 border-t border-[var(--border)] bg-gray-50/30 flex items-center justify-between">
                    <span className="text-sm text-gray-500">
                        Menampilkan <span className="font-semibold text-gray-700">{Math.min((currentPage - 1) * itemsPerPage + 1, filtered.length)}</span> hingga <span className="font-semibold text-gray-700">{Math.min(currentPage * itemsPerPage, filtered.length)}</span> dari total <span className="font-semibold text-gray-700">{filtered.length}</span> relawan
                    </span>
                        ...
                        <div className="px-6 py-4 border-b border-[var(--border)] flex justify-between items-start">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-red-50 border-2 border-red-100 flex items-center justify-center">
                                    <span className="text-[var(--primary)] font-bold text-sm">
                                        {selectedUser.donorProfile?.fullName?.charAt(0) || selectedUser.email.charAt(0).toUpperCase()}
                                    </span>
                                </div>
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">
                                        {selectedUser.donorProfile?.nik ? `NIK: ${selectedUser.donorProfile.nik}` : selectedUser.email}
                                    </p>
                                </div>
                            </div>
                            <button onClick={() => setSelectedUser(null)} className="text-gray-400 hover:text-gray-600 transition-colors p-1">
                                <X className="w-5 h-5"/>
                            </button>
                        </div>
                        ...
                        <div className="px-6 py-4 bg-gray-50/50 border-b border-[var(--border)] grid grid-cols-3 gap-4 text-center">
                            <div>
                                <div className="text-xl font-bold text-[var(--text)]">
                                    ...
                                    {selectedUser.donorProfile?.rhesus === 'POSITIVE' ? '+' : selectedUser.donorProfile?.rhesus === 'NEGATIVE' ? '-' : ''}
                                </div>
                                <div className="text-xs text-gray-500 mt-0.5">Gol. Darah</div>
                            </div>
                            <div>
                                <div className="text-xl font-bold text-[var(--text)]">{selectedUser.donorProfile?.totalDonations ?? 0}</div>
                            ...
                            <div>
                                <div
                                    ...
                                    style={{
                                        backgroundColor: getDonorStatus(selectedUser.donationHistories).bg,
                                        color: getDonorStatus(selectedUser.donationHistories).color
                                    }}
                                >
                                    {getDonorStatus(selectedUser.donationHistories).icon === 'ready'
                                        ? 'Siap Donor'
                                        : getDonorStatus(selectedUser.donationHistories).icon === 'recovery'
                                        ? 'Pemulihan'
                                        : getDonorStatus(selectedUser.donationHistories).icon === 'soon'
                                        ? 'Hampir Siap'
                                        : 'Belum Pernah'}
                                </div>
                        ...
                        <div className="p-6 flex-1 overflow-y-auto">
                            ...
                            {!selectedUser.donationHistories?.length ? (
                                <div className="text-center py-10">
                                    ...
                                    <p className="text-gray-400 text-sm">Belum ada riwayat donasi tercatat.</p>
                                </div>
                            ) : (
                                    ...
                                    {selectedUser.donationHistories.map((log, i) => (
                                        <div key={log.id} className="flex gap-3">
                                            ...
                                            <div className="bg-white border border-[var(--border)] rounded-xl p-3.5 flex-1 mb-0.5 shadow-sm">
                                                <p className="font-semibold text-gray-800 text-sm">{log.locationName}</p>
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {new Date(log.donationDate).toLocaleDateString('id-ID', {
